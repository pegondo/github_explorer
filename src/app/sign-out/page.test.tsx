...
describe("The Sign Out page", () => {
  const setup = async () => {
    render(await SignOutPage());

    const element = screen.getByTestId("sign-out-page");
  ...
  it("should redirect to / if the user isn't logged in", async () => {
  ...
  it("shouldn't redirect if the user is logged in", async () => {
    mockGetServerSession();
